...
export async function GET() {
  ...
  const [callAtt, trainingAtt, meetingAtt, eventAtt] = await Promise.all([
    callIds.length
      ? supabaseDb.from("call_attendance").select("call_id, time_in, time_out, on_my_way_at, anticipated_arrival_at, prep_time_minutes").eq("member_id", memberId).in("call_id", callIds)
  ...
  const result = allActive.map((item) => {
    let att: { time_in: string | null; time_out: string | null; rsvp_at?: string | null; arrived_at?: string | null; on_my_way_at?: string | null; anticipated_arrival_at?: string | null; prep_time_minutes?: number | null } | null = null;

    if (item.type === "call") {
      const r = callAttMap.get(item.id) as any;
      if (r) att = { time_in: r.time_in, time_out: r.time_out, on_my_way_at: r.on_my_way_at, anticipated_arrival_at: r.anticipated_arrival_at, prep_time_minutes: r.prep_time_minutes };
