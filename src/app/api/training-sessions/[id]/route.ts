...
export async function GET(_req: Request, ctx: any) {
  ...
  const { data, error } = await supabaseDb.from("training_sessions").select("*").eq("id", id).single();
...
export async function PATCH(req: Request, ctx: any) {
  ...
  if (body.instructor != null) patch.instructor = body.instructor ? String(body.instructor).trim() : null;
  ...
  if (body.incident_lat !== undefined) patch.incident_lat = body.incident_lat != null && body.incident_lat !== "" ? Number(body.incident_lat) : null;
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  await logActivity(req, "edit_training", { title: data.title });
  return NextResponse.json({ data });
}
...
export async function DELETE(_req: Request, ctx: any) {
  ...
  if (!row?.is_test) return NextResponse.json({ error: "Only TEST trainings can be hard-deleted." }, { status: 403 });

  const { error } = await supabaseDb.from("training_sessions").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
