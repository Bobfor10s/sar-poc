...
function computeStatus(m: MeetingRow): string {
...
export default function MeetingsPage() {
  ...
  async function load() {
    ...
    setMeetings(Array.isArray(json) ? json : (json?.data ?? []));
    setLoading(false);
  }
  ...
  const visible = useMemo(() => {
    return meetings.filter((m) => {
      if (!showArchived && computeStatus(m) === "archived") return false;
      return true;
    });
  ...
  return (
    <main style={{ padding: 24, fontFamily: "system-ui", maxWidth: 860 }}>
      ...
      {loading ? (
        ...
        <p style={{ opacity: 0.7, marginTop: 16 }}>No meetings yet.</p>
      ) : (
